...
export class Display{
    ...
    randomize(){
        this.randommap=JSON.parse(JSON.stringify(this.nextrandommap))
        for(let i = 0; i<10;i++){
            this.nextrandommap[i]=[]
            for(let z = 0;z<12;z++){
                this.nextrandommap[i][z]=Math.random()
            }
        }
    }
    ...
    renderRoom(mr, invisible=false){
        ...
        var r = this.randommap
        if(invisible){
            ctx.translate(-10000,0)
            r=this.nextrandommap
        }
        if(mr.index.background){
            ctx.drawImage(this.game.themes.getImg(mr.theme,mr.background,r[0][0]),0,0,1600,900)
        ...
        if(mr.index.shapes){
            ...
            if(shapes.text){
                for(let i = 0; i<shapes.text.length;i++){
                    var text=shapes.text[i]
                    if(text.background){
                    ...
                    ctx.fillText(text.str,text.x+text.w/2,text.y+text.h/2,text.w)
                }
            }
        }
        ...
        if(mr.doors){
            for(let i = 0; i<mr.doors.length;i++){
                ctx.fillStyle="#000"    
                var door=mr.doors[i]
                ctx.drawImage(this.game.themes.getImg(mr.theme,door.asset,r[1][i]),door.x,door.y,door.w,door.h)
                if(door.location==-1){
                    ctx.lineWidth=10
                    ctx.strokeStyle="#f00"
        ...
        if(mr.index.foreground){
            ctx.drawImage(this.game.themes.getImg(mr.theme,mr.foreground,r[0][0]),0,0,1600,900)
